...
#[test]
fn psychic_damage_and_might_reduce_speed() {
    let mut backend = battle! {
        "Slowpoke" 36 (max ivs, Impish) vs "Metapod" 46 (max ivs, Impish)
    };
    backend.get_pokemon_mut(1).stats[5] = 0;

    test_rng_mut!(backend.rng).force_secondary_effect(1);
    let turn1 = backend.process_turn("Psychic", "Harden");
    ...
    let turn2 = backend.process_turn("Psychic", "Harden");
