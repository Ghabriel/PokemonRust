...
#[test]
fn fire_blast_deals_damage_and_might_burn() {
    let mut backend = battle! {
        "Vulpix" 56 (max ivs, Serious) vs "Krabby" 56 (max ivs, Serious)
    };

    let turn1 = backend.process_turn("FireBlast", "Slam");
    ...
    let turn2 = backend.process_turn("FireBlast", "Slam");
