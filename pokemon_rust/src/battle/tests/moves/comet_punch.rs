...
#[test]
fn comet_punch_deals_damage_two_to_five_times() {
    macro_rules! test {
        (rng = $value:literal yields $expected_number_of_hits:literal hits) => {
            let mut backend = battle! {
                "Hitmonchan" 5 (max ivs, Serious) vs "Metapod" 5 (max ivs, Serious)
            };

            test_rng_mut!(backend.rng).force_custom_multi_hit_value($value);
            let events = backend.process_turn("CometPunch", "Harden");
